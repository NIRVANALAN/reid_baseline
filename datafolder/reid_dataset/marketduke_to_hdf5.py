...
def marketduke_to_hdf5(data_dir, dataset_name, save_dir=os.getcwd()):
    ...
    for phase in phase_list:
        ...
        for i in range(len(phase_dataset['data'])):
            ...
            temp.create_dataset('img', data=Image.open(
                phase_dataset['data'][i][0]))
            temp.create_dataset('index', data=int(phase_dataset['data'][i][1]))
            temp.create_dataset(
                'id', data=phase_dataset['data'][i][2], dtype=dt)
            temp.create_dataset('cam', data=int(phase_dataset['data'][i][3]))

    ids = f.create_group('ids')
    ids.create_dataset('train', data=np.array(
        dataset[0]['ids'], 'S4'), dtype=dt)
    ids.create_dataset('query', data=np.array(
        dataset[1]['ids'], 'S4'), dtype=dt)
    ids.create_dataset('gallery', data=np.array(
        dataset[2]['ids'], 'S4'), dtype=dt)
